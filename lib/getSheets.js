...
var sheetData = {
    ...
    writeToFile: function(qrsInteract, filePath) {
        return qrsInteract.Get("/app/object/count?filter=objectType eq 'sheet'")
            .then(function(countResult) {
                console.log(countResult.body["value"]);

                var dataMatrix = [];
                return getPage(qrsInteract, countResult.body["value"], dataMatrix, pageSize, 0).then(function(dataMatrix) {
                    return writeCSV.writeDataToFile(filePath, dataMatrix);
                }).catch(function(error) {
                    console.log(error);
                });;
            });
